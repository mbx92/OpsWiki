    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-[20px] font-[700] text-[#111111]" style="font-family: 'Manrope', sans-serif;">Add User</h1>
            </div>
        </template>

        <form @submit.prevent="form.post(route('settings.users.store'))" class="max-w-xl space-y-4 rounded-[12px] border border-[#e5e7eb] bg-white p-6">
            <div>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input v-model="form.password" type="password" required class="mt-1.5 block w-full rounded-[8px] border border-[#e5e7eb] px-3 py-2 text-[14px]" />
                    <InputError :message="form.errors.password" />
                    <p class="mt-1.5 text-[13px] text-[#6b7280]">{{ $page.props.saas?.passwordHint }}</p>
                </div>
                <div>
                    <InputLabel value="Confirm password" />
                    <input v-model="form.password_confirmation" type="password" required class="mt-1.5 block w-full rounded-[8px] border border-[#e5e7eb] px-3 py-2 text-[14px]" />
                </div>
            </div>
            <div>
